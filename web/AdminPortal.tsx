  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row text-slate-900 font-sans selection:bg-red-500 selection:text-white">
      ...
      <main className="flex-1 p-8 lg:p-20 overflow-y-auto bg-slate-50">
        ...
        {loading && <div className="animate-pulse text-xs font-black uppercase tracking-widest text-slate-400 mb-8">Querying DB...</div>}
          ...
          <div className="space-y-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              ...
              <div className="bg-white border-8 border-black p-12">
                <p className="text-xs font-black uppercase text-slate-400 mb-2 tracking-[0.2em]">Total Packets</p>
              ...
              <div className="bg-black text-white border-8 border-black p-12">
                <p className="text-xs font-black uppercase text-slate-500 mb-2 tracking-[0.2em]">System Health</p>
                <p className="text-8xl font-black tracking-tighter text-emerald-500 italic">Stable</p>
              </div>
            </div>
          </div>
        )}
              ...
              {inquiries.map(i => (
                <div key={i.id} className="bg-white border-[8px] border-black p-10 flex flex-col lg:flex-row justify-between gap-10 hover:translate-x-4 transition-transform shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
                  <div className="flex-1">
                    ...
                    <div className="mt-6 p-6 bg-slate-100 border-l-8 border-black font-medium italic">"{i.notes || 'Standard protocol.'}"</div>
                  </div>
            ...
            <div className="grid gap-6">
              {monitoringNotes.map(n => (
                <div key={n.id} className={`border-[8px] p-8 ${n.priority === 'CRITICAL' ? 'border-red-600 bg-red-50' : 'border-black bg-white'}`}>
                  ...
                  <p className="text-[10px] font-black uppercase opacity-60">AuthID: {n.author_id.slice(0, 8)}</p>
                </div>
              ))}
            </div>
          </div>
